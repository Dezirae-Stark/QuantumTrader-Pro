...
void ProcessTradingSignals()
{
   ...
   while(signalStart > 0 && StringGetChar(response, signalStart) != '{')
      signalStart--;
   ...
   //--- Extract prediction bounds for SL/TP
      ...
      if(!HasOpenPosition(POSITION_TYPE_BUY))
      {
         ...
         if(trade.Buy(lots, _Symbol, 0, sl, tp, "Quantum Signal"))
         {
            ...
            if(SendTelegramAlerts)
               SendTelegramNotification("📈 BUY Signal Executed on " + _Symbol + " (Confidence: " + DoubleToString(confidence, 1) + "%)");
         }
      }
      else
      {
   ...
   else if(signalType == "SELL" && action == "SELL" && confidence >= 70)
   {
      if(!HasOpenPosition(POSITION_TYPE_SELL))
      {
         ...
         if(trade.Sell(lots, _Symbol, 0, sl, tp, "Quantum Signal"))
         {
            ...
            if(SendTelegramAlerts)
               SendTelegramNotification("📉 SELL Signal Executed on " + _Symbol + " (Confidence: " + DoubleToString(confidence, 1) + "%)");
         }
      }
      else
      {
